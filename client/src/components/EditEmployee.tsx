...
import { addEmployee, updateEmployee, uploadImage } from "../api";
...
const EditEmployee:React.FC<props> = ({setShowModal, showModal, employee, handleToastNotification, setToastMsg, setToastStyles}) => {
  ...
  const [employeeForm, setEmployeeForm] = useState<EmployeeInterface>(employee);

  // Same names used to make sure to be consistent with err response
  const [errors, setErrors] = useState<ErrorsInterface>(initalErrorState);
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if(empImage != null){
      ...
      await updateEmployee(updatedForm)
            .then(res => {
              setToastStyles('bg-blue-600 text-white');
              setToastMsg("Employee data updated successfully!")
  ...
  useEffect(() => { 
    ...
    setCheckedItems({
      BSC:  employee.f_course == 'BSC' ? true : false ,
      BCA: employee.f_course == 'BCA' ? true : false,
      MCA: employee.f_course == 'MCA' ? true : false,
    })
  }, 
    [employee])
  ...
  return (
    <div className="absolute w-screen h-screen top-0 left-0 flex justify-center items-center" style={{backgroundColor : "rgba(0, 0, 0, 0.5"}}>
        ...
        <div className="flex justify-between">
          <h1 className="font-bold text-2xl pb-2 ml-5">Edit Employee</h1>
          ...
          <form className="w-[90%] mx-auto border border-slate-200 rounded-lg p-5">
            <div>
              <div className="flex gap-4 items-center w-full mt-4">
                  ...
                  <div className="w-full ml-7">
                    <input type="text" name="" id="" className="outline-none border border-slate-400 rounded-lg px-3 py-1 w-[85%]" placeholder="Enter Employee name..." onChange={(e) => onInputChange('f_name', e.target.value)} value={employeeForm.f_name}/>
            ...
            <div className="flex gap-4 items-center w-full my-4">
                ...
                <div className="w-full ml-8">
                  <input type="email" name="" id="" className="outline-none border border-slate-400 rounded-lg px-3 py-1 w-[85%]" placeholder="Enter Employee Email..." onChange={(e) => onInputChange('f_email', e.target.value)} value={employeeForm.f_email}/>
            ...
            <div className="flex gap-4 items-center w-full my-4">
                ...
                <div className="w-full">
                  <input type="number" name="" id="" className="outline-none border border-slate-400 rounded-lg px-3 py-1 w-[85%]" placeholder="Enter Employee Mobile No..." onChange={(e) => onInputChange('f_mobile', e.target.value)}value={employeeForm.f_mobile}/>
            ...
            <label htmlFor="countries" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Designation</label>
              <select id="countries" defaultValue="ab" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" onChange={(e) => {onInputChange('f_designation', e.target.value)}}>
                ...
                <option value = "HR" selected={employeeForm.f_designation == 'HR'}>HR</option>
                <option value="Manager" selected={employeeForm.f_designation == 'Manager'}>Manager</option>
                <option value="Sales" selected={employeeForm.f_designation == 'Sales'}>Sales</option>
              </select>
